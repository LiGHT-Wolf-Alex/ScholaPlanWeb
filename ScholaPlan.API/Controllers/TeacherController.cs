...
namespace ScholaPlan.API.Controllers
{
    ...
    public class TeacherController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<Teacher>>> Update(int id, [FromBody] Teacher teacher)
        {
            if (id != teacher.Id)
            {
                _logger.LogWarning($"ID учителя в URL ({id}) не совпадает с ID в теле запроса ({teacher.Id}).");
                return BadRequest(new ApiResponse<Teacher>(false, "Несоответствие ID."));
            }

            if (!ModelState.IsValid)
            {
                _logger.LogWarning("Некорректные данные при обновлении учителя.");
                return BadRequest(new ApiResponse<Teacher>(false, "Некорректные данные."));
            }
            ...
            {
                _logger.LogWarning($"Учитель с ID {id} не найден при обновлении.");
                return NotFound(new ApiResponse<Teacher>(false, "Учитель не найден."));
            }
            ...
            {
                await _unitOfWork.SaveChangesAsync();
                _logger.LogInformation($"Учитель с ID {id} успешно обновлен.");
                return Ok(new ApiResponse<Teacher>(true, "Учитель успешно обновлен.", existingTeacher));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Ошибка при обновлении учителя с ID {id}.");
                return StatusCode(500, new ApiResponse<Teacher>(false, "Внутренняя ошибка сервера."));
            }
        }
        ...
        public async Task<ActionResult<ApiResponse<string>>> Delete(int id)
        {
            ...
            if (teacher == null)
            {
                _logger.LogWarning($"Учитель с ID {id} не найден при удалении.");
            ...
            try
            {
                ...
                await _unitOfWork.SaveChangesAsync();
                _logger.LogInformation($"Учитель с ID {id} успешно удален.");
                return Ok(new ApiResponse<string>(true, "Учитель успешно удален."));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Ошибка при удалении учителя с ID {id}.");
